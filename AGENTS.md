...
- The cross-rail startup invariants (unconditional daemon start, `chromium-ready.target` decoupling, kiosk display-wait fail-open) are pinned by `scripts/test-headless-startup-contract.sh` (run from the root `Makefile` in CI).
