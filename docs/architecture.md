...
- **The kiosk deliberately does not gate on the network** (a blocked kiosk is a black screen): on Wi-Fi boots the player page routinely paints before association completes and its network fetches die without retry, so `feral-controld` performs a one-shot recovery of the player when the boot's first online transition fires: an in-app `refreshArtwork` evaluate (the player re-mounts the current artwork with its own crossfade), escalating to a browser-level `Page.reload` only when the player does not ACK (`MaybeRecoverPlayerOnBootOnline` — wired only when the daemon started within the boot window; driven by sys-monitord's real WAN probe, with the notifier positively matching only the WAN-confirmed transitions (`StateOnline`, or `StateUnprovisioned` with `ReasonUnprovisioned` — its `link-*` legs are offline parking states); latched so connectivity flaps can never disturb playing artwork).
