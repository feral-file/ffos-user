...
| playertoast signature notice | Own single-slot worker over its own per-send CDP session (never the primary client). Parks on `NavigationPending` before each send with the same target-generation exit as §3.2, and re-checks that settled condition plus its display-transition epoch immediately before the evaluate. A notice superseded during the park (the post-navigation generation hook Clears it) is dropped, and so is one whose park times out — a page whose handler never installs cannot render a toast — rather than delivered best-effort. |
