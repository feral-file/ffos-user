...
- `maxConcurrent` (default `16`, used when `> 0`) — requested global in-flight command budget. A command's internal weight is clamped to this budget, so setting it below a heavy command's weight throttles that command (it reserves the whole budget while in flight) rather than rejecting it forever. While the source preflight is enabled, an oversized value is lowered so no more than one worst-case HTTPS `displayPlaylist` preflight is admitted: with the current cast weight of 4, the effective maximum is 4, and controld logs both the configured and effective values when it applies the cap. A probe uses a status-only HTTP/1 client that accepts at most 64 KiB and 128 fields across informational plus final header blocks, discards fields through a fixed reader rather than building a `MIMEHeader` map, and closes before the body so trailers are never parsed. Its shared 8 MiB semaphore has 32 x 256 KiB budget units: HTTP spends one; HTTPS spends four (1 MiB), limits all peer handshake bytes to 64 KiB before certificate parsing, and does not retain TLS connection state. `sourceProbe.disabled: true` removes that derived command cap because casts no longer probe; `commandStorm.disabled: true` removes the gate altogether, while the prober's weighted units remain the final parsing-memory bound.
