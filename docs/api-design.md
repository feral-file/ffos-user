...
`GET /api/screenshot` uses an isolated, short-lived CDP connection so PNG encoding cannot block the persistent command/status connection. `width` and `height` are optional positive integers up to 4096; either may be supplied alone. Two-axis boxes must not exceed 8,294,400 pixels (3840×2160, the 4K-UHD pixel budget); a one-axis request is proportionally scaled down when its derived surface would exceed that budget. This preserves aspect ratio without cropping or stretching while keeping every accepted custom capture deliverable within the 40 MiB PNG and 64 MiB CDP-frame limits. With neither parameter, the native Chromium viewport is returned subject to the same decoded-pixel and response-byte limits. Capture plus response delivery is single-flight and has a 10-second capture deadline; the Hub's 30-second write deadline bounds a backpressured client. The PNG is evidence of Chromium renderer output, not proof of compositor, HDMI, or physical-panel output. The route remains inside the Hub's unauthenticated trusted-LAN boundary; rejecting `Origin` and `Sec-Fetch-Site` requests reduces browser/DNS-rebinding exposure but is not a substitute for the planned shared Hub authorization layer.
