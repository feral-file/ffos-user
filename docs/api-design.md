...
- `player_status` — playback/UI state from Chromium via CDP `checkStatus` (cast command, playlist, pause, etc.). This is not a substitute for hardware or OS-level facts. Includes a numeric `renderStatus` beside `index` carrying the artwork render outcome — `0` pending, `1` loading, `2` ready, `3` failed — which controld relays unchanged rather than interpreting.
