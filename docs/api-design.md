...
The `mintPairingApprovalDecision` command is a controller-to-controld approval response for browser-session mint pairing. It is handled inside `feral-controld`, not forwarded to Chromium. Success and validation failures both return explicit `RPC` payloads with `ok`, `status` or `error.code`, and `approvalRequestID` where available. An approval may carry optional `keepPaired: true`, which mints an owner-kept browser session: `feral-controld` asks `ff-relayer` for `persistent: true` with no `expiresInSeconds`, ignores the browser's `requestedExpiresInSeconds`, and the delivered session carries `persistent: true` with a null `expiresAt` — it ends when the owner removes the site, not on a clock. Without the flag the controld-owned TTL policy applies (default 3600 s, floor 90 s, cap 86400 s). Raw browser session tokens and DP1 playlist content must never appear in this command or in `mint_pairing_approval_request` / `mint_pairing_approval_outcome` relayer messages.
