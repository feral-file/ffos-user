...
- `player_status` — playback/UI state from Chromium via CDP `checkStatus` (cast command, playlist, pause, etc.). This is not a substitute for hardware or OS-level facts. Includes a `renderStatus` beside `index` carrying the artwork render outcome, one of `pending`, `loading`, `ready` or `failed`, which controld relays unchanged rather than interpreting. The player and controld must agree on this value's type: a mismatch fails the whole `player_status` unmarshal, so every poll reports `ok:false` instead of real state.
