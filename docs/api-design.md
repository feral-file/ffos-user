...
- **HTTP layer (this service):** once the redirected probe lands on the routes above, the `302`-on-probe (rather than returning the 204/success body each OS expects) is what makes the phone conclude it is behind a captive portal and auto-open the page. The one exception: after a client fetches the page with a full browser (Safari/Chrome/Firefox, not an OS captive mini-browser), that client's probes get the success body so the OS drops its own sheet — see the captive-portal section of [`setup-flow.md`](setup-flow.md).
