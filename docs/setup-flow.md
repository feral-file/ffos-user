...
| `softap_qr` | AP is up; the phone should join `FF1-<device_id>` and open the portal. Carries SSID, PSK, and the optional direct on-link `portal_url`. After a failed join the re-raise (and the idle re-arm) also carries `reason`, the failure's user-facing message, which the player shows under the title — the `join_failed` paint that precedes it is overwritten within a millisecond, so this is the only place the reason is visible on the device. Painted twice per raise: the raise announcement (join QR), then again with `client_attached: true` once the portal sees the raise's first request — a phone is on the hotspot — so the player swaps the join QR for a `portal_url` QR the phone's still-open camera can scan into its browser (feral-file#3515: iOS holds the captive sheet back while Camera stays in front). When that phone leaves the hotspot (station poll) or goes silent for three minutes, the join QR is painted again, the former with `reason` set to the phone-left line. Players that predate the flag keep the join QR up. |
...
Scanning the QR proposes the AP to the phone; it does not guarantee that the phone joins it. Some phones require a separate Join/Connect confirmation, and others open Wi-Fi settings without selecting the setup SSID. Once the phone joins, its OS issues a captive-portal probe (`/generate_204`, `/hotspot-detect.html`, etc.). The portal answers every probe with a `302` to `/` (rather than the 204/success body the OS expects), which makes the OS decide it is behind a captive portal.

Whether the OS then *shows* the portal page is its own decision. iOS presents the native sheet only while a "Wi-Fi app" (Settings, Safari, a networking app) is in front; a phone that joined from the Camera app's QR scan logs `waiting for UI` until the user switches apps (feral-file#3515, reproduced on two iPhones with both HTTP-redirect and RFC 8908 CAPPORT discovery). The first **Apple** portal request of a raise — the iOS/macOS probe announces itself as `CaptiveNetworkSupport … wispr` — therefore queues an attached-client event on the machine's loop, which re-announces `ap_active` with `ClientAttached` and the raise's credentials. Android never arms it: Google Camera's QR join hands off to Wi-Fi Settings (nothing is looking at the screen), Android posts its own "Tap to sign in" affordance, and a phone with cellular data keeps cellular as its default route while the hotspot is unvalidated, so a browser opened on the hotspot address never reaches it (Pixel trial 2026-09-07; the OS sign-in window works because it binds to the hotspot). Non-Apple traffic still counts as an attached device for the deferral and idle re-arm. The wiring repaints `softap_qr` with `client_attached: true`, and the player swaps the join QR for one encoding `portal_url`: the phone's camera is still pointed at the screen, the new code surfaces a browser link, and opening it both loads the setup page and brings a Wi-Fi app forward. The latch is per raise (cleared on every teardown; the portal's traffic callback is bound to the generation of the raise it was built for, and the event carries it too, so a request or event that outlives its raise touches nothing), so a join-failure re-raise or a recheck blink paints the join QR again, matching the re-association the phone has to make. A raise that never learned its address retries the NetworkManager lookup at the attach and, if still empty, keeps the attach pending and retries on every tick until the address appears or the raise ends — the iOS Camera path sends one probe and then waits for foreground UI, so there is no later probe to retry on, and a missed lookup must not pin a raise on the join QR for its lifetime. While the portal-address QR is up, a goroutine per raise polls the kernel's station list for the AP interface every two seconds (`softap.StationCounter`, nl80211 over generic netlink: the daemon runs as the `feralfile` user, which the system bus policy locks out of every wpa_supplicant method and signal, and the image ships no `iw`; nl80211 reads need no privilege). The poll tracks the phone that attached rather than the size of that list: the source IP of the portal request that raised the address QR is resolved through the kernel's neighbor table (`/proc/net/arp`, another unprivileged read, populated by the hotspot's own DHCP handshake) to that phone's station address, and every reading asks whether THAT address is still associated. Until the table can answer — the first probe can land before the entry is complete — the poll falls back to "is anyone associated", so the aggregate is only ever the interim reading; neither the phone's address on the hotspot nor the hardware address it resolves to is logged, only the fact that the resolution succeeded. Without the correlation a second device on the hotspot masks the first one leaving: the count never reaches zero, and that other device's own portal traffic keeps the silence backstop fresh too. Two consecutive readings without the attached phone mean it left — the sheet cancelled, the user walked away, iOS dropped a network with no internet — and the loop hands the latch back and paints the join QR again with `ClientLeft`, which the player shows as "Your phone left the setup Wi-Fi. Scan the code to join again." under the title; the screen shows the code the next scan needs within about five seconds instead of leaving a code only an associated device can use (design review 2026-09-07). The poll is a side channel: a stale reading (another raise, or landing after the latch cleared) is dropped, the kernel read happens off the loop so a stalled query cannot stall the machine, and five consecutive unknown reads switch the poll off for the raise. Because the out-of-box raise is unbounded and the poll can be off, the latch also re-arms after three minutes of portal silence (`attachedIdleReset`) and the join QR is painted again: the backstop for a phone that went quiet without leaving the AP's station list (a phone out of range stays listed until the AP's inactivity timeout). The silence is measured from the later of the last portal request and the attached-phase paint, so a pending attach whose address arrives late — after the original request has already gone stale — keeps the portal-address QR for a full window instead of being undone by the same tick's idle re-arm.

Opening that link has a second iOS consequence: Safari coming forward is exactly the "Wi-Fi app in front" condition, so iOS launches its native captive sheet on top of Safari (field trial 2026-09-07: sheet registered 80 ms after Safari's foreground, before Safari had fetched anything — so no portal answer can prevent it, and answering that client's probes with the success body only made the sheet display "Success" with a checkmark). The sheet is the better iOS form: it closes itself when the join tears the hotspot down. What is left is the Safari tab underneath on an untouched picker, so the picker page carries a hand-off watcher: while idle it polls `/status` (first poll on load, each poll aborted after 1.5 s, a poll on every return to visibility, and polls made after the hand-off carry `X-Setup-Watcher` so the daemon does not count a page whose user has moved on as an attached device — polls before it are unmarked and do count, since an open picker is a human mid-setup and a recheck blink must not tear the hotspot down under them), and once a join has started (`joining`/`succeeded`) or the device stops answering — three network failures in a row, never an HTTP error, and never while the form holds input or focus — it replaces itself with "Setup continues on your Art Computer … you can close this page". Because the daemon tears the hotspot down BEFORE attempting the join, a wrong password re-raises it; the hand-off page keeps watching and reloads into the picker (with the server's failure banner) as soon as the device answers again in any state but `joining`. A `failed` status reaching a form that holds input or focus keeps it (that is the user's retry in progress); reaching an untouched picker that was rendered before the failure (the page stamps the status it was rendered with in `main[data-status]`) it reloads, because Safari was frozen under the sheet for the whole wrong-password round trip — the reload picks up the server's failure banner, and a picker already showing that banner never reloads on the same persisted outcome.

The page renders a picker from the cached SSIDs after removing the exact active setup SSID. The picker starts on a disabled selection prompt, so no destination network is chosen implicitly. It also provides a manual name field and a "hidden network" checkbox — always both, never either/or (the old picker-only page made hidden networks unprovisionable whenever any network was in range, D3). A non-blank manual entry wins over the picker and is trimmed (phone keyboards autocomplete trailing spaces); picker values pass through verbatim, since leading/trailing bytes are valid SSID content.
