...
Whether the OS then *shows* the portal page is its own decision. iOS presents the native sheet only while a "Wi-Fi app" (Settings, Safari, a networking app) is in front; a phone that joined from the Camera app's QR scan logs `waiting for UI` until the user switches apps (feral-file#3515, reproduced on two iPhones with both HTTP-redirect and RFC 8908 CAPPORT discovery). The first portal request of a raise — probe or page fetch — therefore queues an attached-client event on the machine's loop, which re-announces `ap_active` with `ClientAttached` and the raise's credentials. The wiring repaints `softap_qr` with `client_attached: true`, and the player swaps the join QR for one encoding `portal_url`: the phone's camera is still pointed at the screen, the new code surfaces a browser link, and opening it both loads the setup page and brings a Wi-Fi app forward. The latch is per raise (cleared on every teardown, and the event carries the raise generation so one that outlives its raise repaints nothing), so a join-failure re-raise or a recheck blink paints the join QR again, matching the re-association the phone has to make. A raise that never learned its address skips the repaint (nothing to swap to). Because the out-of-box raise is unbounded, the latch also re-arms after three minutes of portal silence (`attachedIdleReset`) and the join QR is painted again: a phone that dismissed the sheet and dropped the network must not pin an unattended screen on a code only an associated device can use.

Opening that link has a second iOS consequence: Safari coming forward is exactly the "Wi-Fi app in front" condition, so iOS launches its native captive sheet on top of Safari (field trial 2026-09-07: sheet registered 80 ms after Safari's foreground, before Safari had fetched anything — so no portal answer can prevent it, and answering that client's probes with the success body only made the sheet display "Success" with a checkmark). The sheet is the better iOS form: it closes itself when the join tears the hotspot down. What is left is the Safari tab underneath on an untouched picker, so the picker page carries a hand-off watcher: while idle it polls `/status`, and once a join has started (`joining`/`succeeded`) or the device stops answering after having answered, it replaces itself with "Setup continues on your Art Computer … you can close this page". A `failed` status keeps the form — that user retries here.

Opening that link has a second iOS consequence: Safari coming forward is exactly the "Wi-Fi app in front" condition, so iOS launches its native captive sheet on top of Safari — two copies of the form (field trial 2026-09-07: sheet registered 80 ms after Safari's foreground; the user typed into the sheet and Safari was left on the untouched picker). The portal therefore treats a page fetch by a **full browser** (a `Safari/`, `Chrome/`, or `Firefox/` user agent that is not an Android `; wv)` WebView) as that client passing the captive step: from then on that client IP's OS probes get the success body (`Success` HTML for Apple, `204` for Android, the Microsoft strings for Windows) instead of the `302`, the OS marks the network as not captive, and no sheet appears. The captive mini-browsers themselves (iOS/macOS CNA, Android's CaptivePortalLogin) never authorize — they close on their own when the network validates, and authorizing on their fetch would shut the Settings-join user's sheet before the password is typed. Authorization lives on the portal `Server`, so it resets with every raise.
