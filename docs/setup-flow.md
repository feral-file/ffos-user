...
| `join_failed` | Three distinct uses share this state because it is the only player state whose body renders prose. (1) Its namesake: a join failed (wrong password / not found / timeout) and the AP is being re-raised for a retry — carries a reason. (2) The `offline_retrying` narrations (M-0/M-1), painted with the AP **down**: the boot-offline entry ("unable to connect…", "connected but no internet", or the hedged "checking…") and the joined-but-no-internet edges. There the machine's `Message` carries the semantics, and the tick keeps the boot wording truthful as the link comes and goes (a sighted link replaces the setup promise with the no-internet wording; an unknown probe swaps in the hedge; a confirmed-lost link restores the promise). (3) The pre-claim OTA flow when an update ladder fails (fixed prose reason, no AP re-raise — see the narrator-policy bullet below). The player-owned "Wi-Fi join failed" **title** is wrong for uses (2) and (3) — a truthful title needs a new ff-player state; recorded as an open gap (the open half of F-12). |
