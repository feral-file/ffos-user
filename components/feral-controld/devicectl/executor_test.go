...
func TestExecutor_UploadLogs_WithSupportBundleIDFallsBackToLegacySignal(t *testing.T) {
	ts := setup(t)
	defer ts.teardown()
...
func TestExecutor_UploadLogs_WithSupportBundleIDReturnsErrorWhenFallbackFails(t *testing.T) {
	ts := setup(t)
	defer ts.teardown()
	...
	ts.mockDBus.EXPECT().
		...
		Return(errors.New("legacy dbus error"))

	result, err := ts.executor.Execute(ts.ctx, cmd)
	assert.Error(t, err)
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "legacy fallback failed")
}
