...
- `playlistschedule`: owns DP-1 item-level `displayAt` filtering when the playlist opts in with `schedule.byDisplayAt=true`. On cast/URL display it caches the full playlist, sends only the active set to the player, arms a timer for the next `displayAt`, and force-casts (`intent.action=now_display`, no `refresh`) on timer fire, sleep-schedule wake, and CDP (re)connect so cutover is not deferred for remaining artwork duration. Soft `refresh: true` stays on the URL/dynamic refresher path only, except the refresher force-casts when it reconstructs an empty in-memory displayAt cache after a controld restart. The player stays unaware of `displayAt` scheduling.
