...
- `commandrouter` is the command dispatch layer. It has a 3-way routing split:
  ...
  Controld-owned pre-CDP special cases are routed before that split when the browser/player must not see privileged token material:
  - `startMintPairingSession`, `closeMintPairingSession`, and `mintPairingApprovalDecision` route into `mintpairing` so controld can own broker channels, approval notifications, and relayer session creation without exposing browser tokens to Chromium.
  - `listEphemeralSessions` and `revokeEphemeralSession` route into `ephemeralsessions` so controllers can manage relayer browser sessions through the device command path while raw tokens and token hashes stay confined to the relayer.
...
- `ephemeralsessions` owns controller-requested browser-session list/revoke calls to the relayer (`GET /api/ephemeral-sessions?topicID=...` and `DELETE /api/ephemeral-sessions/{sessionID}?topicID=...`). It returns metadata-only command responses and must never expose raw session tokens or token hashes.
