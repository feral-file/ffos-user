...
/// Default kiosk player URL when `ff1-config.json` omits `webapp_url` (device-local ff-player).
/// Remote hosted player (e.g. canary) is opt-in via `webapp_url` in that file.
/// Keep in sync with `components/feral-watchdog/ff1config.DefaultWebappURL`.
