...
func TestSysMetricsPayloadPreservesGpuBusy(t *testing.T) {
	...
	const payload = `{"gpu":{"max_frequency":2200,"current_frequency":2200,"current_temperature":42,"max_temperature":95,"gpu_busy":0}}`
	...
	if metrics.GPU.GPUBusy != 0 {
		t.Fatalf("decoded GPU busy = %v, want 0", metrics.GPU.GPUBusy)
	}
