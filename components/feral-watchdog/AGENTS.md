...
- `ChromiumMonitor` (`chromium.go`) is a long-running background goroutine that polls `http://localhost:9222/json/version` via HTTP (not WebSocket/CDP). Check interval: 5 s. Before polling, it reads DRM connector status via `DisplayDetector` (`display.go`); when display state is known and no monitor is connected, Chromium absence is expected, checks/restarts are skipped, and the monitor resets to pre-connect mode so a later plug-in gets the normal startup grace. Hang detection has two modes:
...
- `SystemdMonitor` (`systemd_service.go`) monitors four systemd services every 30 s: `feral-player.service`, `feral-setupd.service`, `feral-controld.service`, `feral-sys-monitord.service`. It logs state transitions and emits incident metrics to vmagent on failure; it does **not** call `systemctl restart` on these services. `feral-controld.service` and `feral-setupd.service` are `PartOf=chromium-ready.target`: for them the monitor checks whether `chromium-ready.target` is active and whether a monitor is physically connected to tell a genuine failure (target active + monitor connected but service inactive → `Error`) apart from an expected teardown/detached window (target inactive or no monitor connected → `Info`). `feral-player.service` and `feral-sys-monitord.service` are always expected up, so any inactive reading for them is logged at `Error`, even with no monitor connected.
