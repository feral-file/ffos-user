...
- `ChromiumMonitor` (`chromium.go`) is a long-running background goroutine that polls `http://localhost:9222/json/version` via HTTP (not WebSocket/CDP). Check interval: 5 s. Hang detection has two modes:
  ...
  - **Display gating** (`display.go`, `isDisplayConnected`) sits ahead of both modes. On a headless device the kiosk deliberately waits for a display before launching Chromium, so `/json/version` is legitimately absent. While the display reads **KNOWN-disconnected** (any `/sys/class/drm/card*-*/status` is the source, the same one `display-restore.sh` uses), the monitor skips the entire escalation path: no kiosk restart and **no** restart-history accumulation, so a long headless period cannot later trip the reboot budget. Detection **fails open** — an unreadable or absent sysfs layout, or a readable status that is not positively `disconnected` (DRM reports `unknown` for unprobeable connectors), is treated as "connected", preserving all escalation on hardware/CI where the layout or status vocabulary differs; escalation is suppressed only when every connector positively reads `disconnected`. When a display reappears, escalation resumes with a **fresh** startup-grace window (`monitorStart`/`hasEverConnected` re-anchored) so a just-plugged monitor gets the full 90 s, not an instant restart. The sysfs root is injectable (`ChromiumMonitor.drmSysfsRoot`) for tests.
- `SystemdMonitor` (`systemd_service.go`) monitors four systemd services every 30 s: `feral-player.service`, `feral-setupd.service`, `feral-controld.service`, `feral-sys-monitord.service`. It logs state transitions and emits incident metrics to vmagent on failure; it does **not** call `systemctl restart` on these services. `feral-setupd.service` and `feral-controld.service` were formerly `PartOf=chromium-ready.target` and torn down with it; they are now decoupled and run unconditionally at boot, so an `inactive` reading for them is a genuine fault reported as a failure (per-service metric + incident notification), the same as `failed`. The 90 s recent-exit grace in `checkSystemdUserServiceStatus` (a `failed` unit that exited under 90 s ago is treated as still starting) still applies. `feral-player.service` and `feral-sys-monitord.service` keep their prior handling: `failed` is a reported failure, but `inactive` is logged at `Error` only and not escalated to a metric. The obsolete `chromium-ready.target` gating (`isUnitActive`, expected-teardown Info window) has been removed.
