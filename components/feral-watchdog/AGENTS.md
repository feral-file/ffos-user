...
- `SystemdMonitor` (`systemd_service.go`) monitors three systemd services every 30 s: `feral-setupd.service`, `feral-controld.service`, `feral-sys-monitord.service`. Restarts any service that is not active. After a monitored service recovers from `failed` to `active`, it CDP-navigates to the same player URL as `feral-setupd`: `webapp_url` in `/home/feralfile/ff1-config.json` when set (explicit override, often remote for dev/canary), else the device-local default via `ff1config.ResolveWebappURL()`.
