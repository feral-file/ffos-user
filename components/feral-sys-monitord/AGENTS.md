...
- `Connectivity` owns internet-status tracking. Checks connectivity by TCP-dialing in two stages under one 5 s budget: primary `8.8.8.8:443` / `8.8.4.4:443` first; fallback `223.5.5.5:443` (AliDNS) / `1.12.12.12:443` (DNSPod) only if no primary has connected after 1 s or every primary has already failed. First success returns immediately and cancels the rest; one target's failure never cancels another. The fallback exists because the Google pair is blocked on every mainland-China network (feral-file#3539); it is staged, not a peer, so devices outside the mainland do not dial Chinese infrastructure on every probe — do not merge the two lists.
- `SysEventWatcher` owns system event observation. It currently emits exactly **two** events: `gpu_hanging` and `gpu_recover`. These are detected by tailing `journalctl -f -k -g amdgpu`, matching "GPU reset begin" or a "*ERROR* ring ... timeout" line for `gpu_hanging` and "GPU reset ... succeeded" for `gpu_recover` (see `classifyAMDGPUJournalLine` in `event.go`).
